...
import { subscribeToReviews, saveReview } from '../services/dbService';
...
export default function Storefront({ products, onAddToCart, cartCount }: StorefrontProps) {
  ...
  const triggerAddAnimation = (product: Product, quantity: number, e?: React.MouseEvent) => {
    ...
    setCartToast({ name: product.name, quantity });
    setTimeout(() => {
      setCartToast(null);
    }, 2500);
  };
  ...
  const handleAddReview = async (e: React.FormEvent, targetProdId?: string) => {
    e.preventDefault();
  ...
  const handleQuickAdd = (product: Product, e?: React.MouseEvent) => {
    ...
    triggerAddAnimation(product, 1, e);
    setTimeout(() => setJustAddedId(null), 1500);
  };

  const handleModalAdd = (product: Product, e?: React.MouseEvent) => {
    onAddToCart(product, modalQuantity);
    triggerAddAnimation(product, modalQuantity, e);
  ...
  return (
    <div className="w-full">
      ...
      {/* Flying Cart Item Animation Overlay */}
        ...
        <AnimatePresence>
          {flyingItems.map(item => (
            <motion.div
              ...
              className="fixed top-0 left-0 w-14 h-14 rounded-2xl bg-gradient-to-tr from-emerald-950 via-emerald-900 to-amber-500 text-gold-300 shadow-2xl border-2 border-gold-400 flex items-center justify-center font-bold text-xs pointer-events-none"
            >
              <div className="flex flex-col items-center justify-center text-center">
                <ShoppingCart className="w-6 h-6 text-gold-300" />
                <span className="text-[7px] font-black text-white truncate max-w-[42px]">
        ...
        <AnimatePresence>
          {cartToast && (
            <motion.div
              ...
              className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-emerald-950/95 backdrop-blur-md text-white px-5 py-3 rounded-2xl shadow-2xl border-2 border-gold-500/60 flex items-center gap-3.5 pointer-events-auto"
            >
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gold-500 to-amber-600 text-emerald-950 font-black flex items-center justify-center shadow-md text-lg shrink-0">
                🛒
              </div>
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-black text-gold-400 uppercase tracking-wider">Item Added to Cart!</span>
                  <span className="bg-gold-500/20 text-gold-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-gold-500/30">
                    +{cartToast.quantity}
                  </span>
                </div>
                <p className="text-xs font-semibold text-gray-200 truncate max-w-[220px] mt-0.5">
                  {cartToast.name}
                </p>
              </div>
            </motion.div>
