...
import { loginAdminWithEmail, logoutAdmin, subscribeToAuth } from '../services/dbService';
...
import heroBanner from '../assets/images/malik_branded_hero_1784633050272.jpg';
import brandLogo from '../assets/images/malik_real_logo_1784634645165.jpg';
...
export default function AdminDashboard({
  ...
  // Legacy PIN UI is kept only to avoid breaking saved UI state; it never grants access.
  ...
  const [loginMode, setLoginMode] = useState<'firebase' | 'passcode'>('firebase');
...
const [storeName, setStoreName] = useState(
  storeSettings?.storeName || 'Malik Oil Expellers'
);

const [tagline, setTagline] = useState(
  storeSettings?.tagline || 'Pure Cold-Pressed Oils & Organic Livestock Feed'
);

const [phone, setPhone] = useState(
  storeSettings?.phone || '0300-7618236'
);

const [whatsapp, setWhatsapp] = useState(
  storeSettings?.whatsapp || '923007618236'
);

const [email, setEmail] = useState(
  storeSettings?.email || 'info@malikoil.pk'
);

const [address, setAddress] = useState(
  storeSettings?.address ||
    'Tandila Road, Samundri, District Faisalabad, Punjab, Pakistan'
);

const [announcementText, setAnnouncementText] = useState(
  storeSettings?.announcementText ||
    '🚚 Free Express Shipping across Pakistan on orders above Rs. 3,500! | 100% Pure Organic Cold-Pressed Oils'
);

const [showAnnouncement, setShowAnnouncement] = useState(
  storeSettings?.showAnnouncement ?? true
);

const [heroTitle, setHeroTitle] = useState(
  storeSettings?.heroTitle || 'Purity Direct From Our Expellers'
);

const [heroSubtitle, setHeroSubtitle] = useState(
  storeSettings?.heroSubtitle ||
    'Traditional wood-pressed natural oils and high-protein livestock feed directly from our family mills in Samundri & Vehari.'
);

const [heroImageUrl, setHeroImageUrl] = useState(
  storeSettings?.heroImageUrl || heroBanner
);

const [logoUrl, setLogoUrl] = useState(
  storeSettings?.logoUrl || brandLogo
);

const [estYear, setEstYear] = useState(
  storeSettings?.estYear || '1970'
);

const [standardShippingRate, setStandardShippingRate] = useState(
  storeSettings?.standardShippingRate ?? 200
);

const [freeShippingThreshold, setFreeShippingThreshold] = useState(
  storeSettings?.freeShippingThreshold ?? 3500
);

const [taxRatePercent, setTaxRatePercent] = useState(
  storeSettings?.taxRatePercent ?? 18
);

const [customNoticeText, setCustomNoticeText] = useState(
  storeSettings?.customNoticeText ||
    '⚡ Special Seasonal Offer: Get 10% instant discount on bulk Mustard Feed Bags!'
);

const [showCustomNotice, setShowCustomNotice] = useState(
  storeSettings?.showCustomNotice ?? true
);
...
const [settingsSavedToast, setSettingsSavedToast] = useState(false);
  ...
  const handleFirebaseLogin = async (e: React.FormEvent) => {
    ...
    const passToUse = adminPassword;

    try {
      await loginAdminWithEmail(emailToUse, passToUse);
      setIsAuthenticated(true);
    } catch (err: any) {
      setAuthError(err?.message || 'Authentication failed. Please check your Firebase admin credentials.');
    } finally {
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center bg-[#FAF9F5] p-4 font-sans">
        <motion.div
          ...
          className="bg-white rounded-3xl p-8 max-w-md w-full border border-gray-100 shadow-xl text-center space-y-6"
        >
          ...
          <div className="space-y-2">
            <h2 className="font-serif font-extrabold text-xl text-emerald-950">MALIK OIL STORE ADMIN LOGIN</h2>
          ...
          {loginMode === 'firebase' ? (
            <form onSubmit={handleFirebaseLogin} className="space-y-4 text-left">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2.5 px-3.5 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-gold-500"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 bg-emerald-950 hover:bg-emerald-900 text-gold-500 hover:text-white font-serif font-extrabold text-xs tracking-widest rounded-xl transition-all cursor-pointer shadow-md uppercase flex items-center justify-center gap-2"
              >
            ...
            <form onSubmit={handleVerifyPin} className="space-y-4">
              ...
              {pinError && (
                ...
                >
                  PIN login is disabled. Please use Firebase Authentication.
                </motion.p>
              )}

              <button
                type="submit"
                className="w-full py-3.5 bg-emerald-950 hover:bg-emerald-900 text-gold-500 hover:text-white font-serif font-extrabold text-xs tracking-widest rounded-xl transition-all cursor-pointer shadow-md uppercase"
              >
          ...
          <p className="text-[10px] text-gray-400">
            Malik Oil Expellers & Spares • Store Management Portal
          </p>
        </motion.div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans relative">
      ...
      <div className="flex flex-wrap gap-2 mb-8 bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.02, y: -1 }}
          whileTap={{ scale: 0.98 }}
          className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-black tracking-wide transition-all cursor-pointer bg-emerald-900 text-amber-300 hover:bg-emerald-800 border border-emerald-700 shadow-sm"
        >
          <Globe className="w-4 h-4 text-amber-400" />
          <span>CLOUD & VERCEL GUIDE (فائر بیس اور ورسل گائیڈ)</span>
        </motion.button>
      </div>
                        ...
                        <tr key={order.id} className="hover:bg-gold-50/20 transition-colors">
                          ...
                          <td className="py-4 px-4 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <button
                                onClick={() => {
                                  if (window.confirm(`Kya aap invoice ${order.invoiceNo} (${order.customerName}) ko mukammal delete karna chahte hain?`)) {
                                    if (onDeleteOrder) {
                                      onDeleteOrder(order.id);
                                    }
                                  }
                                }}
                                className="px-2 py-1.5 bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white rounded font-bold text-[10px] tracking-wide transition-all cursor-pointer flex items-center gap-1 border border-rose-200"
                ...
                <div className="bg-white rounded-3xl max-w-lg w-full p-6 md:p-8 shadow-2xl border border-gray-100 space-y-6">
                  ...
                  <div className="border-t border-gray-100 pt-3 flex justify-end">
                    <button
                      type="button"
                      onClick={() => {
                        if (window.confirm(`Kya aap invoice ${selectedOrderDetails.invoiceNo} (${selectedOrderDetails.customerName}) ko HAMESHA ke liye delete karna chahte hain?`)) {
                          if (onDeleteOrder) {
                            onDeleteOrder(selectedOrderDetails.id);
                          }
                          setSelectedOrderDetails(null);
                        }
                      }}
                      className="px-4 py-2 bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center gap-2 border border-rose-200"
                    >
                      <Trash2 className="w-4 h-4" />
                      <span>Delete This Order Permanently (آرڈر ڈلیٹ کریں)</span>
                    </button>
                  </div>
                </div>
              </div>
      ...
      {/* CLOUD & VERCEL SETUP GUIDE MODAL */}
      <AnimatePresence>
        {showCloudSetupModal && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 border border-gray-100 shadow-2xl space-y-6 text-gray-800"
            >
              <div className="flex justify-between items-center border-b pb-4 border-gray-100">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-950 text-gold-500 rounded-2xl flex items-center justify-center font-bold">
                    <Globe className="w-5 h-5 text-amber-400" />
                  </div>
                  <div>
                    <h3 className="font-serif font-extrabold text-lg text-emerald-950">
                      Firebase Cloud Database & Vercel Deployment Guide
                    </h3>
                    <p className="text-xs text-gray-500">
                      Malik Oil Expellers Cloud Architecture & Vercel Hosting
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-xs text-emerald-950 uppercase tracking-wide">
                    Firebase Cloud Database Connected & Active
                  </h4>
                  <p className="text-xs text-emerald-800 mt-0.5">
                    Your products, orders, customers, and settings are synchronized live via Firebase Firestore across all mobile & desktop devices.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <h4 className="font-extrabold text-xs text-emerald-950 uppercase tracking-wider flex items-center gap-2">
                  <Zap className="w-4 h-4 text-amber-500" />
                  1. Firebase Firestore Configuration
                </h4>
                <div className="bg-gray-900 text-gray-100 rounded-2xl p-4 font-mono text-xs overflow-x-auto space-y-1">
                  <p className="text-emerald-400">// Automatically loaded from firebase-applet-config.json</p>
                  <p><span className="text-amber-400">Project ID:</span> {firebaseConfig.projectId || 'db6c7f8d-63f9-4c80-ac5c-f6f8711c0fae'}</p>
                  <p><span className="text-amber-400">Auth Domain:</span> {firebaseConfig.authDomain || 'db6c7f8d-63f9-4c80-ac5c-f6f8711c0fae.firebaseapp.com'}</p>
                  <p><span className="text-amber-400">Database ID:</span> (default)</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/products</span>
                    <span className="text-gray-600 text-[11px]">Real-time catalog, stock levels & pricing</span>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/orders</span>
                    <span className="text-gray-600 text-[11px]">Live order placements & status sync</span>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/customers</span>
                    <span className="text-gray-600 text-[11px]">Customer purchase history & directories</span>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/settings</span>
                    <span className="text-gray-600 text-[11px]">Store CMS, banners, and contact info</span>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/discountCodes</span>
                    <span className="text-gray-600 text-[11px]">Active promotional discount codes</span>
                  </div>
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="font-bold text-emerald-900 block font-mono">/reviews</span>
                    <span className="text-gray-600 text-[11px]">Customer ratings & reviews sync</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2 border-t border-gray-100">
                <h4 className="font-extrabold text-xs text-emerald-950 uppercase tracking-wider flex items-center gap-2">
                  ...
                  3. Vercel Deployment Steps (ورسل پر ڈیپلائے کرنے کا طریقہ)
                </h4>
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs space-y-2 text-amber-950">
                  <p className="font-bold">Follow these steps to deploy to Vercel:</p>
                  <ol className="list-decimal list-inside space-y-1 text-[11px]">
                    <li>Connect your GitHub repository (<span className="font-mono bg-amber-100 px-1 rounded">MalikUsmanAbid/malik-oil-store</span>) to Vercel.</li>
                    <li>Select <span className="font-bold">Vite</span> as Framework Preset in Vercel settings.</li>
                    <li>Ensure Build Command is <span className="font-mono bg-amber-100 px-1 rounded">npm run build</span>.</li>
                    <li>Ensure Output Directory is <span className="font-mono bg-amber-100 px-1 rounded">dist</span>.</li>
                    <li>The <span className="font-mono bg-amber-100 px-1 rounded">firebase-applet-config.json</span> configuration file is included in your source code, so Firebase sync works out-of-the-box on Vercel without requiring extra server environment variables!</li>
                  </ol>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end">
                <button
                  onClick={() => setShowCloudSetupModal(false)}
                  className="px-6 py-2.5 bg-emerald-950 text-gold-500 font-bold text-xs rounded-xl hover:bg-emerald-900 transition-colors cursor-pointer"
                >
                  CLOSE GUIDE
                </button>
              </div>
